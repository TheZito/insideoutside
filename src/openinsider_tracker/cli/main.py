...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="openinsider_tracker")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "ingest":
        from openinsider_tracker.cli.commands.ingest import run_ingest

        return run_ingest(args, config)
    if args.command == "classify":
        from openinsider_tracker.cli.commands.classify import run_classify

        return run_classify(args, config)
    if args.command == "notify":
        from openinsider_tracker.cli.commands.notify import run_notify

        return run_notify(args, config)
    if args.command == "thresholds":
        from openinsider_tracker.cli.commands.thresholds import run_thresholds

        return run_thresholds(args, config)
    if args.command == "serve":
        from openinsider_tracker.cli.commands.serve import run_serve
